...
/*
 * ScrollCraft v2 for oalfawzan.sa
 * Scroll is the timeline. This deliberately avoids CSS ScrollTimeline as the
 * primary mechanism so iOS/mobile gets the same experience as desktop.
 */
(() => {
  ...
  style.textContent = `
    ...
    .sc-act { --sc-p:0; --sc-local:0; position:relative; }
    ...
    /* Pin/stack device: the card becomes the stage while the next beat arrives. */
    @media (max-width:760px) and (prefers-reduced-motion:no-preference) {
      html[data-sc-page="home"] .projects-section .project-card,
      html[data-sc-page="blog"] .blog-card {
        position:sticky;
        top:calc(92px + min(var(--sc-index,0), 5) * 7px);
    ...
    /* Timeline device: draw is controlled from JS via --sc-line-p. */
  ...
  if ('IntersectionObserver' in window) {
    ...
    active = new Set(observed);
  }

  const visibleProgress = (el, vh) => {
  ...
  const updateTimeline = (vh) => {
    ...
    const nodes = [...timeline.querySelectorAll('.tl-item')];
  ...
  const updatePhotos = (vh) => {
    ...
    active.forEach(el => {
      ...
      const depth = (parseInt(el.style.getPropertyValue('--sc-index') || '0', 10) % 3) - 1;
  ...
  /* Pointer depth is an enhancement only; mobile does not depend on it. */
